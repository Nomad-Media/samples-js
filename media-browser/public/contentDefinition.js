...
await getContentDefinition();

async function getContentDefinition()
{
    const LOWER_PATH = window.location.pathname.substring(1);
    const PATH = LOWER_PATH.charAt(0).toUpperCase() + LOWER_PATH.slice(1);

    const MEDIA_GROUP = await sendRequest(`/get${PATH}`, "GET");
...
async function createFeaturedGroupsRows(FEATUED_GROUPS)
{
    for (let featuredGroup of FEATUED_GROUPS)
    {
        ...
        for (let item of featuredGroup.items)
        {
            ...
            VIDEO_DIV.appendChild(FEATURED_GROUP_HEADER);
            VIDEO_DIV.appendChild(FEATURED_GROUP_TABLE);
        }
    }
}
