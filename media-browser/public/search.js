...
const SEARCH_RESULTS_DIV = document.getElementById("searchResultsDiv");
...
SEARCH_FORM.addEventListener('submit', async function (event) {
    ...
    FORM_DATA.append('search', SEARCH_INPUT.value);

    const SEARCH_RESULTS = await sendRequest(`/getSearch`, "POST", FORM_DATA);

    SEARCH_DIV.hidden = true;

    await createSearchResultRow(SEARCH_RESULTS);
});
...
async function createMenuItems()
{
    ...
    FORM_DATA.append('id', CONFIG.landingPageMediaGroup.id);

    const MEDIA_GROUP = await sendRequest('/getMediaGroup', 'POST', FORM_DATA);
    const MEDIA_GROUP_MENU_ITEMS = MEDIA_GROUP.mediaGroupMenuItems;
...
async function createSearchResultRow(SEARCH_RESULTS)
{
    const SEARCH_RESULT_TABLE = document.createElement('table');
    SEARCH_RESULT_TABLE.classList.add("table");
    const SEARCH_RESULT_ROW = SEARCH_RESULT_TABLE.insertRow();

    for (let item of SEARCH_RESULTS)
    {
        const SEARCH_RESULT_CELL = SEARCH_RESULT_ROW.insertCell();
        SEARCH_RESULT_CELL.innerText = item.title;

        SEARCH_RESULT_CELL.classList.add("rowSelect");

        SEARCH_RESULT_CELL.addEventListener('click', async function (event) {
            await enterDialog(item.itemId);
        });
                        
        SEARCH_DIV.appendChild(SEARCH_RESULT_TABLE);
    }

    SEARCH_RESULTS_DIV.appendChild(SEARCH_RESULT_TABLE);
}
